				...
				<el-row :gutter="20">
					...
					<el-col :span="12">
						<el-form-item label="生活补贴" prop="ab_ex40">
							<el-input v-model="dibaoForm.ab_ex40" placeholder="请输入生活补贴"></el-input>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row :gutter="20">
					<el-col :span="12">
						<el-form-item label="生活补贴标准" prop="ab_ex38">
							<el-input v-model="dibaoForm.ab_ex38" placeholder="请输入生活补贴标准"></el-input>
						</el-form-item>
					</el-col>
				</el-row>
		...
		methods: {
			...
			loadData(){
				...
				this.getBasic({
					...
					this.isFinish = true
					console.log(this.isFinish,1111)
				})
				// 共同生活
				this.loadFamily({
					sId: this.$route.query.sId,
			...
			save(){
				...
				for(let i in this.familyData1){
					...
					if(abm_ex2){
						...
						this.$message.warning("第"+(index)+"行共同家庭成员请选择是否是保障对象，只有保障对象才计算保障金额");
						isSave=false;
						break;
					}
				...
				for(let i in this.familyData3){
					this.familyData3[i].abm_ex6 = "A"
					let abm_ex2=this.familyData3[i].abm_ex2;
					let index=Number(i)+1;
					if(abm_ex2){
						let msg=this.isCardID(abm_ex2);
						if(msg){
							this.$message.warning("第"+(index)+"行监护人情况"+msg);
							isSave=false;
							break;
						}else{
							this.familyData1[i].abm_ex4=this.getBirthdayFromIdCard(abm_ex2);
