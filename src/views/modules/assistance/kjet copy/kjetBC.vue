<template>
  <div class="g-box">
    ...
    <div class="search">
      <el-form :inline="true" :model="searchData" class="demo-form-inline">
        ...
        <el-form-item label-width="110px" label="是否健康">
          <el-select
            v-model="searchData.whetherHealth"
            placeholder="请选择是否健康"
          >
            <el-option
        ...
        <el-form-item label-width="110px" label="是否残疾">
          <el-select
            v-model="searchData.whetherDisability"
            placeholder="请选择是否残疾"
          >
            <el-option
              v-for="item in whethers"
        ...
        <el-form-item label-width="110px" label="是否重病">
          <el-select
            v-model="searchData.whetherSeriousIllness"
            placeholder="请选择是否重病"
          >
            <el-option
              v-for="item in whethers"
        ...
        <el-form-item label-width="110px" label="残疾类型">
          <el-select
            v-model="searchData.disabilityType"
            placeholder="请选择残疾类型"
          >
            <el-option
    ...
    >
      ...
      ></el-table-column>
      <el-table-column
        prop="basicLivingMoney"
        label="低保金"
        width="100px"
      ></el-table-column>
      <el-table-column
        prop="disabledSubsidyMoney"
        label="残疾人生活补贴费"
        width="150px"
      ></el-table-column>
      <el-table-column
      ...
      <el-table-column prop="active" label="操作" :width="cazuoBtn">
        <template slot-scope="scope">
          <el-button-group>
            ...
            <el-button
              v-if="isFinish"
              @click="handleClick(scope.row, '救助转移')"
              type="danger"
              size="small"
              >转移</el-button
            >
            <el-button
              v-if="isFinish"
              @click="handleClick(scope.row, '打印档案')"
              type="primary"
              size="small"
              >打印档案</el-button
            >
            ...
            <el-button
              v-if="isFinish"
              @click="handleClick(scope.row, '变更')"
              type="danger"
              size="small"
              >变更</el-button
            >
            <el-button
              v-if="isFinish"
              @click="handleClick(scope.row, '注销')"
              type="danger"
              size="small"
              >注销</el-button
            >
    ...
    handleClick(data, key) {
      if (key == "查看") {
        ...
        if (process.env.NODE_ENV == "development") {
          ...
          this.$router.push({
            path:
              "/social/assistance/kjet/kjetListView?type=审核&id=" + data.id,
          });
        } else {
          this.$pushRouter({
            path:
        ...
        if (process.env.NODE_ENV == "development") {
          ...
          this.$router.push({
            path:
              "/social/assistance/kjet/kjetListView?type=审批&id=" + data.id,
          });
        } else {
          this.$pushRouter({
            path:
        ...
        if (process.env.NODE_ENV == "development") {
          ...
          // grade 信息显示等级 1：区（市）2：街道； confirmationDate 选择的年月
          this.$router.push({
            path: "/social/assistance/kjet/kjetRescueTransfer?sId=" + data.id,
          });
        } else {
          this.$pushRouter({
            path:
              generalConfig.defaultUrl +
              "/social/assistance/kjet/kjetRescueTransfer",
          });
        }
      } else if (key == "变更") {
        if (process.env.NODE_ENV == "development") {
          ...
          // grade 信息显示等级 1：区（市）2：街道； confirmationDate 选择的年月
          this.$router.push({
            path: "/social/assistance/kjet/kjetChangeView?sId=" + data.id,
          });
        } else {
          this.$pushRouter({
            path:
              generalConfig.defaultUrl +
              "/social/assistance/kjet/kjetChangeView",
          });
        }
      } else if (key == "注销") {
        if (process.env.NODE_ENV == "development") {
          ...
          this.$router.push({
            path:
              "/social/assistance/kjet/kjetListView?type=注销&id=" + data.id,
          });
        } else {
          this.$pushRouter({
            path:
