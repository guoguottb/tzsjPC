  	...
  	<div class="g-box"> 
		...
		<el-tabs v-model="activeName" @tab-click="handleClickSelect">
		  <el-tab-pane  v-for="(item, index) in options2"
			...
			style="width: 190" @selection-change="handleSelectionChange" height="100%" v-loading="loading">
			<el-table-column prop="selecttions" type="selection" width="15"> </el-table-column>
			<el-table-column prop="changeId" label="儿童ID" width="15"></el-table-column>
			<el-table-column prop="name" label="儿童姓名" width="15"></el-table-column>
			<el-table-column prop="idCard" label="儿童身份证" width="30"></el-table-column>
			<el-table-column prop="gender" label="性别" width="15"> </el-table-column>
			<el-table-column prop="age" label="年龄" width="15"> </el-table-column>
			<el-table-column prop="healthStatus" label="健康状态" width="20"></el-table-column>
			<el-table-column prop="typeName" label="儿童类型" width="15"></el-table-column>
			<el-table-column prop="money" label="救助金额" width="15"></el-table-column>
			<el-table-column prop="applyName" label="申请人姓名" width="15"></el-table-column>
			<el-table-column prop="childCode" label="儿童编号" v-if="false"></el-table-column>
			<el-table-column prop="childId" label="儿童ID" v-if="false"></el-table-column>
			<el-table-column prop="familyCode" label="家庭编号" v-if="false"></el-table-column>
			<el-table-column prop="active"  label="操作" width="60">		
    ...
    methods:{
		...
		handleClick(data,key){
            if(key=='查看'){
				...
				if(process.env.NODE_ENV == "development"){  //生产环境执行
					// grade 信息显示等级 1：区（市）2：街道； confirmationDate 选择的年月
					this.$router.push({path: '/social/assistance/kjet/kjetChangeListView?type=审核&sId='+data.childId+'&changeId='+data.changeId,});
				}else {
					this.$pushRouter({path:generalConfig.defaultUrl + '/social/assistance/kjet/kjetChangeListView',})
				}
			}else if(key =='审批'){
				if(process.env.NODE_ENV == "development"){  //生产环境执行
					// grade 信息显示等级 1：区（市）2：街道； confirmationDate 选择的年月
					this.$router.push({path: '/social/assistance/kjet/kjetChangeListView?type=审批&sId='+data.childId+'&changeId='+data.changeId,});
				}else {
					this.$pushRouter({path:generalConfig.defaultUrl + '/social/assistance/kjet/kjetChangeListView',})
				}
			}
