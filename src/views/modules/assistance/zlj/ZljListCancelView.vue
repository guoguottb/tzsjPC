		...
		<div class='g-box2'>
			...
			<el-form :inline="true" class="demo-form-inline">
				...
				<div class="top_1">
					<table class="top_2">
						...
						<!-- <tr style="height:40px;">
							...
							<td style="width:10%;">预警时间:</td>
							<td style="width:20%;">{{warningData[1].aw_dat1.slice(0,10)}}</td>
						</tr>
						<tr style="height:40px;">
							<td style="width:10%;">预警内容:</td>
							<td style="width:20%;">{{warningData[2].aw_ex1}}</td>
							<td style="width:10%;">预警时间:</td>
							<td style="width:20%;">{{warningData[2].aw_dat1.slice(0,10)}}</td>
						</tr> -->
					</table>
				</div>
                ...
                <div v-if="type!='zx'" style='margin-top:15px'>
                    ...
                    <div class="lineSt"></div>
                    <div class="top_1">
                        <div style="color:#6292c2;font-weight:bold;padding:5px;margin-left:5%;">
							注销原因：
							<span style="color:red;">{{basicInf.ac_ex8}}</span>
						</div>
                        <div style="color:#6292c2;font-weight:bold;padding:5px;margin-left:5%;">
							申请注销时间：
							<span id="ac_dat1_2">{{basicInf.ac_dat1}}</span>
						</div>
                    </div>
                </div>
					...
					<el-row :gutter="20" style='margin-top:10px'>
						...
						<el-col :span="12">
							<el-card class="box-card">
								...
								<!-- {{basicInf.ab_ex16.split(',')[0]=="" ? 0 : basicInf.ab_ex16.split(',').length}} -->
								</div>
					...
					<el-row :gutter="20" style='margin-top:10px'>
							...
							<el-card class="box-card">
								<div slot="header" class="clearfix">
									<span>审核审批表(<span
										...
										<!-- {{basicInf.ab_ex74.split(',')[0]=="" ? 0 : basicInf.ab_ex74.split(',').length}} -->
						...
						<el-col :span="12">
							<el-card class="box-card">
								<div slot="header" class="clearfix">
									<span>电子授权书(<span
								style='color:red'>{{imgNum.ab_ex73_len}}</span>)</span>
								
								<!-- {{basicInf.ab_ex73.split(',')[0]=="" ? 0 : basicInf.ab_ex73.split(',').length}} -->
					...
					<el-row :gutter="20" style='margin-top:10px'>
						<el-col :span="12" >
							<el-card class="box-card">
								<div slot="header" class="clearfix">
									<span>公示图片(<span
										style='color:red'>{{imgNum.ab_ex98_len}}</span>)</span>
				...
				<div class="top_1" v-if="this.type == 'zx'">
					...
					<el-row style="margin-top: 10px;" type="flex" justify="center">
						<el-col :span="24" style="text-align: center;">
							<el-button type="primary" @click="save2(publicity)" :disabled='disabled'>保存</el-button>
							</el-col>
					</el-row>
				</div>
				<br>
		...
		<div class="ainfo2">
			...
			<div style="margin-left:30px;margin-top:20px;">
				<ul>
					...
					<li :class="{listStyle:isShow18}" @click="isShow(18)" style="height:25px">
						<a class="ainfoFont" :class="{listStyleA:!isShow18}"  @click="counter('#page18')">代理人信息</a>
					</li>
	...
	export default {
		...
		methods: {
			scroll(){
				...
				if(!oDiv1 || !oDiv2 || !oDiv3 || !oDiv4 || !oDiv5 || !oDiv6 || !oDiv7 || !oDiv8 || !oDiv9 || !oDiv10|| !oDiv12|| !oDiv13|| !oDiv14|| !oDiv15|| !oDiv16|| !oDiv17|| !oDiv18|| !oDiv19|| !oDiv20|| !oDiv21 || !oDiv22){
					return ''
				}
			...
			counter(id) { //counter1是绑定的点击事件名称，点击右边菜单时跳转到该区域
			...
			loadData(){
				...
				this.getData({
					...
					this.loadWarning({
						...
						// 过滤预警空数据
						this.warningData = res.response.data[0].waring.filter((item, index, arr)=> {
							return item.aw_dat1 != "" || item.aw_ex1 != ""
						})
						console.log(this.warningData,"预警");
						// if(res.response.data[0].length>0){
