		...
		methods: {// changeAmt(index,row,id)
				...
				this.getBasicsChange({
					...
					this.getBasicIncome({
							...
							for(let i in res.response.data[0].data ){
								...
								// res.response.data[0].data[i].abi_ex2=(res.response.data[0].data[i].abi_ex2).toFixed(2)
			...
			for(var key in data[0]){
				...
				if(old!=val&&key.indexOf("abm_")==-1&&key.indexOf("abp_")==-1&&key.indexOf("_id")==-1&&key.indexOf("ac_")==-1){
					...
					if(key=="ab_ex10"||key=="ab_ex16"||key=="ab_ex73"||key=="ab_ex97"||key=="ab_ex74"||key=="ab_ex98"||key=="ab_ex166"||key=="ab_ex167"){
						var olds=old.split(",");
						var vals=val.split(",");
						old="";
