		...
		<div class='g-box2'>
			...
			<el-form :model="dibaoForm" :rules="rules" ref="dibaoForm" label-width="auto" style='padding-right: 10px;'>
				...
				<el-table :data="familyData2" border :row-class-name="rowClassName"
					...
					<el-table-column prop="abm_ex21" label="家庭月总收入" min-width="130">
						<template slot-scope="scope">
							<el-input v-model="familyData2[scope.row.xh-1].abm_ex21"></el-input>
						</template>
					</el-table-column>
					<el-table-column prop="abm_ex20" label="家庭总人口" min-width="120">
						<template slot-scope="scope">
							<el-input v-model="familyData2[scope.row.xh-1].abm_ex20"></el-input>
						</template>
					</el-table-column>
					<el-table-column prop="abm_ex18" label="月赡抚扶养费" min-width="130">
						<template slot-scope="scope">
				...
				<el-row :gutter="20">
					...
					<el-col :span="12">
						<el-form-item label="生活补贴" prop="ab_ex38">
							<el-input v-model="dibaoForm.ab_ex38" placeholder="请输入生活补贴"></el-input>
						</el-form-item>
					</el-col> 
					<!-- <el-col :span="12">
			...
			loadData() {
				...
				this.getBasicsChange({
					...
					if(res.response.data[0].msg=="该信息正在进行变更申请，不能进行变更操作"){
						...
						let xzqhArray=dibaoForm.ab_ex162.split(",");
			...
			save() {
				    ...
				    for(var key in data[0]){
				        ...
				        if(old!=val&&key.indexOf("abm_")==-1&&key.indexOf("abp_")==-1&&key.indexOf("_id")==-1&&key.indexOf("ac_")==-1){
							...
							name=key=="ab_ex154" ? "公示图片":name;
				            if(key=="ab_ex10"||key=="ab_ex16"||key=="ab_ex73"||key=="ab_ex97"||key=="ab_ex74"||key=="ab_ex98"||key=="ab_ex154"){
				                var olds=old.split(",");
				                var vals=val.split(",");
				                old="";
