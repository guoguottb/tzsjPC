<template>
	<div class="g-box" v-loading="loading">
		<div class='g-box2'>
			<div class="info2">
				<el-form :inline="true" class="demo-form-inline">
					...
					<el-dialog title="转办数据完善" center :visible.sync="dialogChangeForm" :before-close="giveUpZB" z-index=1999  width=80% class="change_type">
						...
						<el-table :data="tableData" border :row-class-name="rowClassName" @selection-change="handleFamilyData1"  empty-text='空' class="my-table">
					...
					<template v-if="this.basicInf.ab_ex15=='办结' && ['特困供养'].includes(publicity.ab_ex13)">
						<h3 class="line_blue" id="page25" style="margin-top: 20px;">&emsp;第三方特困自理能力复核信息</h3>

						<div class="lineSt"></div>
						<div style="position: relative;">
							<el-table :data="this.tableData" border style="width: 100% ; margin-bottom: 20px;">
								...
								<el-table-column label="复核结果" min-width="180">
			...
			<div class="ainfo2">
				...
				<div style="margin-left:30px;margin-top:20px;">
					<ul>
						...
						<!-- <li :class="{listStyle:isShow24}" @click="isShow(25)" style="height:25px">
							<a class="ainfoFont" :class="{listStyleA:!isShow24}"  @click="counter('#page24')">汇总表</a>
						</li> -->
			...
			<!-- 自理复核结果表 -->
			<el-dialog title="自理复核结果" :visible.sync="detailShow2" top='30vh' center>
				<el-table :data="pgjg" border>
					<el-table-column property="aer_ex4" label="特困人员" width="100"></el-table-column>
					<el-table-column property="aer_ex15" label="评估得分" width="80"></el-table-column>
					<el-table-column property="aer_ex12" label="复核结果" width="100"></el-table-column>
					<el-table-column property="aer_dat3" label="评估完成时间" width="auto"></el-table-column>
					<el-table-column property="aer_ex31" label="评估人员" width="auto"></el-table-column>
					<el-table-column property="aer_ex32" label="评估机构" width="100"></el-table-column>
				</el-table>
			</el-dialog>
	...
	export default {
		...
		data() {
			return {
				...
				detailShow2:false, // 自理复核结果展示
				detailShow3:false, // 自理复核结果PDF列表展示
		...
		methods: {
			...
			openPGPDF(data){
				...
				// }else{
					...
					.catch((res)=>{
						...
						console.log( res.response.data,'复核结果列表');

						if(res.response.data.length==0){
							this.$alert('暂无复核结果pdf');
						}else{
			...
			changeChildrenMoney(abm_ex3){
				...
				// 	// 	this.tableData[i].abm_ex71 = this.childrenWarning[j].abm_ex71;
				// 	// 	this.tableData[i].abm_ex69 = this.childrenWarning[j].abm_ex71;
				// 	// 	this.tableData[i].abm_ex72 = 'Y';
						...
						if(data.ab_ex207 == '是'){// 渐退期
							...
							for(let i in this.tableData){
								...
								if(this.tableData[i].abm_ex11 == '重病' ){
									...
									// 兴化儿童--调标后废弃
									// if(this.tableData[i].abm_ex51=='未成年人' && xinghu_ertong){
									// 	//兴化儿童资金赋值
									// 	if(this.tableData[i].abm_ex72!=''){
									// 		detailObj.abd_ex4 = this.tableData[i].abm_ex71;
									// 		detailObj.abd_ex8 = 0;
									// 	}
									// 	// 成员数据金额赋值
									// 	if( this.tableData[i].abm_ex71 == ''){
									// 		this.tableData[i].abm_ex71 = (detailObj.abd_ex4 * 1 + detailObj.abd_ex8 * 1).toFixed(2);
									// 	}
								...
								// 低保重残
									...
									// 	// 兴化儿童--调标后废弃
									// 	if(this.tableData[i].abm_ex72!='Y'){
									// 		detailObj.abd_ex4 = this.tableData[i].abm_ex71;
									// 		detailObj.abd_ex8 = 0;
									// 	}
									...
									if(['退役军人','归侨','独居','老年人','未成年人'].includes(this.tableData[i].abm_ex51)){
										...
										// //  兴化儿童--调标后废弃
										// if(this.tableData[i].abm_ex51 == '老年人' || this.tableData[i].abm_ex51 == '未成年人'){
										// 	console.log('兴化儿童',this.tableData[i].abm_ex51);

										// 	detailObj.abd_ex7 =  this.tableData[i].abm_ex51;
										// 	if(detailObj.abd_ex7=='未成年人' && this.tableData[i].abm_ex72 !='' && xinghu_ertong){
										// 		// 兴化儿童修改
										// 			detailObj.abd_ex4 = this.tableData[i].abm_ex71;
										// 			detailObj.abd_ex8 = '';
										// 			console.log('兴化儿童修改==>');
										...
										// 	}
										// }else if(['退役军人','归侨','独居'].includes(this.tableData[i].abm_ex51)){
