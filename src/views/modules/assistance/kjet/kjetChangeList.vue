<template>
  	<div class="g-box"> 
		...
		<el-table  ref="table" :data="tableData" tooltip-effect="dark" border 
			...
			<el-table-column prop="applyName" label="申请人姓名" width="100px"></el-table-column>
			<el-table-column prop="childCode" label="儿童编号" width="100px" v-if="false"></el-table-column>
			<el-table-column prop="childId" label="儿童ID" width="100px" v-if="false"></el-table-column>
			<el-table-column prop="familyCode" label="家庭编号" width="100px" v-if="false"></el-table-column>
		...
		handleClick(data,key){
            if(key=='查看'){
				if(process.env.NODE_ENV == "development"){  //生产环境执行
					...
					this.$router.push({path: '/social/assistance/kjet/kjetEtzrListView?id='+data.childId,});
				}else {
					this.$pushRouter({path:generalConfig.defaultUrl + '/social/assistance/kjet/kjetEtzrListView',})
				}
            }else if(key=='提交审核'){
